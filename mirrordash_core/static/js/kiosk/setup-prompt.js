...
class SetupPrompt extends HTMLElement {
        ...
        this.shadowRoot.innerHTML = `
            <style>
                :host {
                    ...
                    /* Define color tokens locally to prevent WebKit inheritance bugs with all: initial */
                ...
                .card {
                    ...
                    border-radius: 1rem;
                    padding: 2.5rem;
                    border: 1px solid var(--color-dimmed-charcoal);
                    text-align: center;
                }
                .icon {
                ...
                .subtext {
                    font-size: 0.8125rem;
                    color: var(--color-dimmed-charcoal);
                    line-height: 1.5;
                    margin: 0;
                }
            </style>
    ...
    updateState(data) {
        ...
        if (data.wifi_hotspot_active) {
            ...
            if (!currentIcon || currentIcon.getAttribute('data-lucide') !== 'wifi') {
                iconContainer.innerHTML = '<i data-lucide="wifi"></i>';
            }
            ...
            showPrompt = true;
            const currentIcon = iconContainer.querySelector('i, svg');
            if (!currentIcon || currentIcon.getAttribute('data-lucide') !== 'monitor-smartphone') {
                iconContainer.innerHTML = '<i data-lucide="monitor-smartphone"></i>';
            }
