...
- Implemented fast multi-threaded ramdisk compression without warnings by relying on the native `pigz` fallback built into `initramfs-tools` when `COMPRESS=gzip` and the `pigz` package is installed.
- Updated `RELEASING.md` to outline clear release and deployment workflows for both the Python Core App and the System OS Golden Image, emphasizing their distinct lifecycles, risks, and deployment mechanics.
- Added Rule 31 to `AGENTS.md` requiring a Table of Contents (TOC) for markdown documents containing more than 3 level-2 (`##`) headings, and generated corresponding TOC headers across `AGENTS.md`, `ARCHITECTURE.md`, `DESIGN.md`, `GOLDEN_IMAGE.md`, `RELEASING.md`, and `USER_GUIDE.md`.
