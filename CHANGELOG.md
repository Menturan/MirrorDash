...
- Fixed a bug in `admin.py` and `backup.py` where update checks, upgrades, and backup exports were checking and installing the obsolete package name `mirrordash-core` (version 0.0.1) instead of the active PyPI package name `mirrordash`.
- Corrected emulated OS build target filenames in `RELEASING.md` to map to `mirrordash-os-vX.Y.Z.img.gz` (generated by `build_image.sh`) and clarified that live card extractions write to `mirrordash-final.img.gz`.
...
[Unreleased]: https://github.com/Menturan/MirrorDash/compare/v0.2.4...HEAD
